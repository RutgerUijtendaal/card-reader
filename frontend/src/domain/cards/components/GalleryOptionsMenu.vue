...
import PopoverOptionList, { type PopoverOptionItem } from '@/domain/cards/components/PopoverOptionList.vue';
import { useFloatingPopover } from '@/shared/composables/useFloatingPopover';
import { HOVER_MODE_OPTIONS, type HoverMode } from '@/domain/cards/utils/gallery/hoverMode';
import { CARD_PAGE_SIZE_OPTIONS } from '@/domain/cards/utils/gallery/pageSize';
import { GALLERY_CARD_SCALE_MAX, GALLERY_CARD_SCALE_MIN, GALLERY_CARD_SCALE_STEP } from '@/domain/cards/composables/useGalleryOptions';
...
const hoverModeMenuOptions = computed<PopoverOptionItem[]>(() =>
  hoverModeOptions.map((option) => ({
    value: option.value,
    label: option.label,
    hotkey: hoverModeHotkeys[option.value],
  })),
);
