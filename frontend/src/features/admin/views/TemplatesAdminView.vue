          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <template v-if="!createMode">
              ...
              <button
                class="btn-secondary"
                type="button"
                :disabled="queueingReparse || !reparseSourceTemplateId"
                @click="queueBulkReparse"
              >
                {{ queueingReparse ? 'Queueing Reparse...' : 'Queue Reparse Into This Template' }}
              </button>
            </template>
          </div>
...
import TemplatePreviewPane from '@/features/admin/components/TemplatePreviewPane.vue';
import {
  createTemplate,
  deleteTemplate,
  fetchTemplates,
  queueTemplateReparse,
  updateTemplate,
...
const templateDefinitionHint = `Define parser-driven regions with region_id, cut_region, parser_type, and ocr_config. name_mana_cost regions may also define mana_badge_ocr. Supported parser types: ${TEMPLATE_PARSER_TYPES_HINT}.`;
...
const queueingReparse = ref(false);
const reparseSourceTemplateId = ref('');
...
const loadTemplates = async (): Promise<void> => {
  ...
  if (rows.length === 0) {
    ...
    reparseSourceTemplateId.value = '';
    resetForm();
    return;
  }
  if (!rows.some((row) => row.key === reparseSourceTemplateId.value)) {
    reparseSourceTemplateId.value = rows[0].key;
  }
...
const queueBulkReparse = async (): Promise<void> => {
  if (!selectedId.value || !reparseSourceTemplateId.value || queueingReparse.value) return;
  queueingReparse.value = true;
  try {
    const result = await queueTemplateReparse(selectedId.value, reparseSourceTemplateId.value);
    toast.success(result.message);
  } catch (error) {
    console.error('Queue template reparse failed', error);
    toast.error(extractErrorMessage(error, 'Failed to queue template reparse.'));
  } finally {
    queueingReparse.value = false;
  }
};
