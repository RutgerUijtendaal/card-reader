...
describe('resolveRouteViewKey', () => {
  ...
  test('does not key routed components by workspace context', () => {
    expect(resolveRouteViewKey('/cards')).toBeUndefined();
    expect(resolveRouteViewKey('/imports')).toBeUndefined();
    expect(resolveRouteViewKey('/my/decks/new')).toBe('deck-editor:/my/decks/new');
  });
});
