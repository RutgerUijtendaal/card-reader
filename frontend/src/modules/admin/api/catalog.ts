...
export const fetchSuggestionDetail = async (
  ...
  const response = await api.get<SuggestionRecord>(`/admin/suggestions/${kind}/${id}`);
  return response.data;
};
...
export const acceptSuggestionToExisting = async (
  ...
  await api.post(`/admin/suggestions/${kind}/${id}/accept`, payload);
};
...
export const rejectSuggestion = async (kind: SuggestionKind, id: string): Promise<void> => {
  await api.post(`/admin/suggestions/${kind}/${id}/reject`);
};
