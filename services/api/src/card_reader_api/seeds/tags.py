...
from card_reader_core.models import Tag, now_utc
from card_reader_core.repositories.helpers import normalize_slug_key

from .shared import resolve_seed_file

DEFAULT_TAGS_FILE = resolve_seed_file("seed-tags.json")
logger = logging.getLogger(__name__)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Seed tag records")
    parser.add_argument(
        "--file",
        type=Path,
        default=DEFAULT_TAGS_FILE,
        help="Path to a tag JSON file (array of labels or tag objects)",
    )
    return parser.parse_args()


def read_tag_entries(file_path: Path) -> list[dict[str, object]]:
    if not file_path.exists():
        logger.warning("Tag seed file not found; skipping default tag seed. file=%s", file_path)
        return []
    try:
        payload = json.loads(file_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError:
        logger.exception("Tag seed file is invalid JSON. file=%s", file_path)
        return []
    if not isinstance(payload, list):
        logger.warning("Tag seed json must be an array. file=%s", file_path)
        return []
...
def seed_tags(seed_file: Path = DEFAULT_TAGS_FILE) -> tuple[int, int]:
    entries = normalize_entries(read_tag_entries(seed_file))
    if not entries:
        return 0, 0
    existing_by_key = {row.key: row for row in Tag.objects.filter(key__in=[key for key, _, _ in entries])}
    created = 0
    updated = 0
    for key, label, identifiers_json in entries:
        existing = existing_by_key.get(key)
        if existing is None:
            Tag.objects.create(key=key, label=label, identifiers_json=identifiers_json)
            created += 1
            continue
...
def main() -> None:
    ...
    args = parse_args()
    initialize_database()
    created, updated = seed_tags(args.file)
    print(f"Tag seed complete. file={args.file} created={created} updated={updated}")
