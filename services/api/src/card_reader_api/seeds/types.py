...
from card_reader_core.models import Type, now_utc
from card_reader_core.repositories.helpers import normalize_slug_key

from .shared import resolve_seed_file

DEFAULT_TYPES_FILE = resolve_seed_file("seed-types.json")
logger = logging.getLogger(__name__)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Seed type records")
    parser.add_argument(
        "--file",
        type=Path,
        default=DEFAULT_TYPES_FILE,
        help="Path to a type JSON file (array of labels or type objects)",
    )
    return parser.parse_args()


def read_type_entries(file_path: Path) -> list[dict[str, object]]:
    if not file_path.exists():
        logger.warning("Type seed file not found; skipping default type seed. file=%s", file_path)
        return []
    try:
        payload = json.loads(file_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError:
        logger.exception("Type seed file is invalid JSON. file=%s", file_path)
        return []
    if not isinstance(payload, list):
        logger.warning("Type seed json must be an array. file=%s", file_path)
        return []
...
def seed_types(seed_file: Path = DEFAULT_TYPES_FILE) -> tuple[int, int]:
    entries = normalize_entries(read_type_entries(seed_file))
    if not entries:
        return 0, 0
    existing_by_key = {row.key: row for row in Type.objects.filter(key__in=[key for key, _, _ in entries])}
    created = 0
    updated = 0
    for key, label, identifiers_json in entries:
        existing = existing_by_key.get(key)
        if existing is None:
            Type.objects.create(key=key, label=label, identifiers_json=identifiers_json)
            created += 1
            continue
...
def type_table_has_rows() -> bool:
    return Type.objects.exists()
...
def main() -> None:
    ...
    args = parse_args()
    initialize_database()
    created, updated = seed_types(args.file)
    print(f"Type seed complete. file={args.file} created={created} updated={updated}")
