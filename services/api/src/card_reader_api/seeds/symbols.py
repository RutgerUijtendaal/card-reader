...
def read_symbol_entries(seed_file: Path = DEFAULT_SYMBOLS_FILE) -> list[SymbolSeedEntry]:
    ...
    for item in payload:
        ...
        json_detection_config = json.loads(raw_detection_config) if isinstance(raw_detection_config, str) else {}
        detection_config_json = raw_detection_config if isinstance(json_detection_config, dict) else {}
