...
Card collections accept one pool plus independent role, faction, and mana-family inclusion/exclusion filters with `any` or `all` matching. Player, Evil, and Neutral are public primary site workspaces selected from the sidenav; the active workspace supplies the Gallery pool and is serialized in shareable Gallery URLs. Player is the default, all three options are always available, and the saved workspace survives login and logout. No workspace has a default role, faction, or mana-family constraint, and Neutral is never added as an implicit overlay.
