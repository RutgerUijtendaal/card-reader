...
Import classification initializes only new card identities. Untargeted imports with known factions match latest image hashes, primary names, and aliases inside the selected pool and exact canonical faction set. An empty Evil faction result instead means unknown: after ordinary empty-namespace matching, Core searches factioned Evil Cards by every historical image checksum and by normalized primary names and aliases. It reuses a Card only when neither source is ambiguous and both sources agree when present. The existing Card keeps its Card-tab pool, roles, and factions. Ambiguous, conflicting, or unmatched results stay independent as transitional no-faction Cards with an actionable `evil_faction_unresolved` warning. Targeted reparses, other pools, known-faction imports, and disabled reparse matching remain namespace-scoped.
