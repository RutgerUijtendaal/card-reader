...
Staff can export the current Player, Evil, or Neutral Gallery selection to TTS. Persistent sheets are partitioned by pool so restricted artwork never shares a public Player atlas. Player sheet images remain public; Evil and Neutral export payloads carry signed, checksum-bound sheet URLs that authorize Tabletop Simulator to fetch only the exact rendered revisions in that export. Deck TTS exports remain Player-only alongside the current deck workflow.
