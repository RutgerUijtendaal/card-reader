...
Each unpublished draft has a stable UUID. Create sends that UUID as an owner-scoped idempotency key, so a retry returns the same deck instead of creating a duplicate. If the initial response is ambiguous, the editor reconciles the UUID with delayed retries; a found deck completes navigation, while repeated misses or unavailable lookups keep the exact attempted payload locked for a safe Retry. A miss unlocks the draft only when the failed Create returned a definitive HTTP response.
