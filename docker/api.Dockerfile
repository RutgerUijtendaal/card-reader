...
CMD ["sh", "-c", "uv run --frozen --package card-reader-api python manage.py migrate_card_reader && uv run --frozen --package card-reader-api gunicorn card_reader_api.project.wsgi:application --pythonpath src --bind 0.0.0.0:8000 --timeout 300"]
