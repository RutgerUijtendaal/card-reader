...
- On Windows, treat a sandboxed `gh` "invalid token" result as possible Windows Credential Manager isolation ([openai/codex#21821](https://github.com/openai/codex/issues/21821)), not immediate proof that GitHub authentication has expired. Prefer the GitHub connector where it supports the operation. For `gh`-only work, retry the same command outside the sandbox with a narrowly scoped approval, and only report an authentication failure if that retry also fails. Never run `gh auth login` or persist a plaintext token as an automatic workaround.
