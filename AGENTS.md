...
- The target card classification model has three independent card-level dimensions:
  ...
  - The only untargeted cross-faction image fallback is manual-correction recovery: a prior completed import of the same image in the same pool must record the currently inferred faction set, the Card must now belong to a different faction namespace, and exactly one Card may have that provenance. Preserve the Card's corrected classification, emit the normal classification-mismatch warning, and refuse the fallback when provenance is absent or ambiguous.
